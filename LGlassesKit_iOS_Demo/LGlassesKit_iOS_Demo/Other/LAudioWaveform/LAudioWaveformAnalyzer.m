...
@interface LAudioWaveformAnalyzer ()
...
@implementation LAudioWaveformAnalyzer
...
- (instancetype)initWithFFTSize:(int)fftSize sampleRate:(NSUInteger)sampleRate channels:(NSUInteger)channels {
    if (self == [super init]) {
        _fftSize = fftSize;
...
- (void)comminit {
    ...
    for (NSUInteger i = 0; i < self.channels; i++) {
    ...
    LBandsInfo *first = [LBandsInfo createWith:self.startFrequency upperFrequency:0];
    for (int i = 1; i <= self.frequencyBands; i++) {
        ...
        [tmps addObject:[LBandsInfo createWith:first.lowerFrequency upperFrequency:first.upperFrequency]];
...
- (float)findMaxAmplitude:(LBandsInfo *)band amplitudes:(NSArray *)amplitudes bandWidth:(float)bandWidth {
...
@implementation LBandsInfo

+ (instancetype)createWith:(float)lowerFrequency upperFrequency:(float)upperFrequency {
    LBandsInfo *info = [[LBandsInfo alloc] init];
